...
const apiPort = 8082;
const apiUrl = `http://${apiHost}:${apiPort}/estore/api/electroitem`;
...
function renderItems(items) {
    ...
    $('.edit-btn').on('click', function() {
        ...
        $('#quantity').val(row.find('td').eq(4).text());
        $('#archive').val(row.find('td').eq(5).text() === 'Да' ? 'true' : 'false');
        $('#description').val(row.find('td').eq(6).text());
        $('#create-btn').hide();
        $('#update-btn').show();
    });
