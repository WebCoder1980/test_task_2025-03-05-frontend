...
const apiPort = 8082;
const apiUrl = `http://${apiHost}:${apiPort}/estore/api/purchase`;
...
$(document).ready(function() {
    ...
    $('#create-btn').on('click', function() {
        const item = {
            ...
            shopId: $('#shopId').val(),
            purchaseDate: $('#purchaseDate').val()+':00',
            type: $('#type').val()
        };
        createItem(item);
    });
...
function renderItems(items) {
    ...
    $('.edit-btn').on('click', function() {
        ...
        $('#shopId').val(row.find('td').eq(3).text());
        $('#purchaseDate').val(row.find('td').eq(4).text().slice(0, 16));
        $('#type').val(row.find('td').eq(5).text());
        $('#create-btn').hide();
        $('#update-btn').show();
    });
